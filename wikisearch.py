...
wikisearch=Blueprint("wikisearch", __name__, static_folder="static", template_folder="templates")
...
@wikisearch.route('/wikisearch', methods=['GET', 'POST'])
def index():
    ...
    if request.method == 'POST':
      ...
      for i in results:
          ...
          if(len(ans)==0):
              continue
          result.append(ans)
          return render_template("wikisearch.html",result = result)
          break
      
      result.append("We are sorry, this model requires precise question")
      return render_template("wikisearch.html",result = result)
